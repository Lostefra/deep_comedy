...
# Splits a texting along word boundaries (empty spaces and punctuation marks). If synalepha is True, doesn't split
...
def prettify(text, keep_diacritics=True):
	if keep_diacritics:
		out = _textip_spaces(_textip_punctuaction(text.lower()))
	else:
		out = _textip_spaces(_textip_punctuaction(_remove_diacritics(text.lower())))
	return out


# Removes hash characters from a texting.
def textip_hashes(text):
	return re.sub("#", "", text)


# Determines if a split between two words is acceptable,
# ie. if there are no synalepha nor elision (eg. "l' amico" should be kept together).
# Heuristic: All apotextophes are considered a non-breakable point.
#            This is not always the case (eg. "perch’ i’ fu’" should be split into "perch’ i’"-"fu’).
def _is_split_acceptable(text, synalepha=False):
	prev = text[0]
	succ = text[len(text) - 1]
	vowel = re.compile(r"""[AEIOUaeiouàèéìòóùÈ]""")
	apotextophe = re.compile(r""".*['`‘’].*""")
	newline = re.compile(r""".*\n+.*""")

	out = newline.match(text) or not (apotextophe.match(text) and (vowel.match(prev) or vowel.match(succ)))

	if synalepha:
		out = out and not (vowel.match(prev) and vowel.match(succ))

	return out


# Removes punctuation from a texting.
def _textip_punctuaction(text):
	return re.sub(r"""[,.;:"“”!?«»—'`’]+""", "", text)


# Removes diacritic marks from a texting.
...
# Removes spaces from a texting.
def _textip_spaces(text):
