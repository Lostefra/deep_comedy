...
# Determines if a word is tronca (accent on the last syllable).
# Exact cases: word ending with an accented letter (morì) or word ending with a consonant (mangiàr).
# Heuristic: NO other words are considered tronche since the majority of Italian words
#            are piane (accent on the second to last syllable) or sdrucciole (third to last).
def is_tronca(word):
...
# Computes a score based on how many letters match from the end of the two strings,
# up to the last vowel of the first vocoid (eg. "men#te" vs. "can#te" tries to match ente and ante,
# computing a score of 0.75, while "iuo#la" vs. "suo#la" tries to match ola and ola, computing a score of 1.0).
# HEURISTIC: since no accent is known, the match is as PERMISSIVE as possible
#            (ie. matches from the LAST vowel of a diphthong).
#            This rhymes correctly "quivi/sorgivi" (while a restrictive heuristic wouldn't).
#            The computed score is the sum of all matching characters (truncated at the first difference),
#            weighted exponentially with the distance from the putative beginning of the rhyme.
#            As such, it's a score which can scale well on different matching lengths
#            (eg. "più" and "fu" have a score similar to "frangente" and "assolutamente"),
#            at the expenses of not having a "natural" meaning which could be easier to threshold.
def _match_syllable(s1, s2):
    # Inside a syllable vowels can only be together, so only the NEXT character needs to be checked.
    lastvowel = re.compile(r"""[aeiou](?![aeiou])""")
